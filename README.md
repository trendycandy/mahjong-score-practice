...
- **점수표 연습 모드**: 헤더 「점수표 연습」 버튼. 점수표 칸(친/자 · 론/쯔모 · 판 · 부)을 단계별(자 론 30부 → 25/50부 → 40부 → 파생 부수 → 쯔모 → 친 → 만관 이상)로 출제하고 점수를 입력하면 채점 + 암기 팁 해설(같은 부수 열, 같은 점수 칸)을 보여준다. 틀린 칸은 세션 안에서 더 자주 나온다.
