...
- **점수표 연습 모드**: 헤더 「점수표 연습」 버튼 → **연습 / 실전** 선택. 연습은 단계별(자 론 30부 → 25/50부 → 40부 → 파생 부수 → 쯔모 → 친 → 만관 이상) 출제 + 범위 조정 + 힌트 보기, 실전은 점수표 전 칸 무작위·힌트 없음. 점수를 입력하면 채점 + 암기 팁 해설(같은 부수 열, 같은 점수 칸). 틀린 칸은 세션 안에서 더 자주 나온다.
